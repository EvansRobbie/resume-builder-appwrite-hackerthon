...
const ListPersonalDetails = () => {
  const [personalDetails, setPersonalDetails] = useState<personalProps | null>(null);
  const userId = localStorage.getItem('userId');
  // console.log(userId)
  useEffect(() => {
      const fetchPersonalDetails = async () => {
        try{
          const response = await databases.getDocument(databaseId, personalCollectionId, userId );
...
console.log(personalDetails)
  return (
    <>
      ...
      {personalDetails  &&  (
       <div key={personalDetails?.$id} className="flex flex-col items-center py-8">
